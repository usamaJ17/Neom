                ...
                <td class="room-column">
                    <div class="d-flex w-100 flex-wrap gap-2">
                        ...
                        @foreach ($rooms as $room)
                            ...
                            @if ($booked)
                                <button class="btn btn--danger btn-sm room-btn" room="room-{{ str_replace(' ','',$room->room_number) }}" disabled>Bed Name : {{ $room->room_number }} (Room No : {{$room->room->bed_name}})</button>
                                @php
                            ...
                            @if($room->roomStatus)
                                ...
                                <button class="btn btn--dark btn-sm room-btn" room="room-{{ str_replace(' ','',$room->room_number) }}" disabled>
                                        Bed Name : {{ $room->room_number }} (Room No : {{$room->room->bed_name}})
                                    </button>
                                @elseif($room->roomStatus->status == 'Under Maintenance')
                                <button class="btn btn--warning btn-sm room-btn" room="room-{{ str_replace(' ','',$room->room_number) }}" disabled>
                                        Bed Name : {{ $room->room_number }} (Room No : {{$room->room->bed_name}})
                                    </button>
                                @else
                                <button class="btn btn--primary btn-sm room-btn available" room="room-{{ str_replace(' ','',$room->room_number) }}" data-room="{{ $room }}" data-date="{{ $checkIn->format('m/d/Y') }}" data-booked_status="0">
                                    Bed Name : {{ $room->room_number }} (Room No : {{$room->room->bed_name}})
                                </button>
                                @endif
                            @else
...
(function($) {
    ...
    function enableDisableBooking() {
        ...
        if (limitCross || lowFromLimit) {
            ...
            alertBox.html('@lang("Please select exactly one room.")');
        } else {
            if (!alertBox.hasClass('alert-info')) {
                alertBox.removeClass('alert-danger').addClass('alert-info');
            }
            alertBox.html("@lang('Every room can be selected or deselected by a single click. Ensure only one room is selected.')");
        }
    }
})(jQuery);
</script>
